...
public:
	NotExpectedLexemeException(const std::string& expected, const Lexeme& resultLexeme)
	{
		if (resultLexeme.type == LexemeType::End)
			...
			message = "Ожидалось " + expected + ", получено \"" + resultLexeme.str + "\" (" + std::to_string(resultLexeme.pos.row) + " ; " + std::to_string(resultLexeme.pos.column) + ")";
	}
};
...
public:
	ExpectedExpressionException(const Lexeme& lexeme)
	{
		if (lexeme.type == LexemeType::End)
			...
			message = "Ожидалось выражение, получено \"" + lexeme.str + "\" (" + std::to_string(lexeme.pos.row) + "; " + std::to_string(lexeme.pos.column) + ")";
	}
};
